...
async def search_venues(query: str, db: AsyncSession = Depends(get_db), limit: int = 20):
    """
    Busca establecimientos por nombre o dirección.
    Ideal para la barra de búsqueda de la aplicación móvil.
    """
    ...
    result = await db.execute(
        ...
        .where(
            or_(
                Venue.nombre.ilike(f"%{query}%"),
                Venue.direccion.ilike(f"%{query}%"),
                Venue.categoria.ilike(f"%{query}%")
            )
        )
        .limit(limit)
    )
